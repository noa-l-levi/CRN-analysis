...
There are additional functions to analyse the subnetworks. In partiuclar, polynomials in selected variables that exist in the rowspan or the ideal generated by the rate equations can be found. 
...
This function finds polynomials in the rowspan of a CRN with monomial terms corresponding to chosen complexes in the CRN

INPUT[2] : The name of a Catalyst-defined reaction network, a vector containing the indices of the chosen complexes

OUTPUT[2] : Vectors representing the linear combinations of rate equations required to reach the rowspan polynomial and the coefficients of each monomial in the rowspan polynomial. There should be zero's corresponding to the non-chosen complexes. 


ideal_polynomials.jl:

This function 

INPUT[2]: The name of a Catalyst-defined reaction network, a vector containing the indices of the selected (not-to-be-eliminated) species

OUTPUT[3]: A Groebner basis for the ideal generated by the rate equations of the reaction network (w.r.t the elimination block ordering (degrevlex(non-chosen species) x degrevlex(chosen species)), the elimination ideal, and the combination of rate equations required to reach the polynomials in the elimination ideal.
